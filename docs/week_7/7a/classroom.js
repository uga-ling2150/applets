...
async function submit(run){if(c.readOnly||c.mode!=='student'||sent.has(run.id))return;pending=run;c.submitting=true;app.controls();$('class-retry').hidden=true;$('class-submission').textContent='Submitting this attempt…';
 try{const d=await api(path('/submit'),'POST',{run});sent.add(run.id);c.submitted=true;pending=null;app.status('Attempt submitted successfully.');$('class-submission').textContent=`Receipt: ${c.room} · P${d.participant} · A${d.attempt||1} · ${d.submittedAt?new Date(d.submittedAt).toLocaleString():'submission time unavailable'}. Keep this to identify your attempt.`;$('class-receipts').textContent+=('\n'+c.room+' · P'+d.participant+' · A'+d.attempt+' · '+(d.submittedAt?new Date(d.submittedAt).toLocaleString():'time unavailable'));fail('');await refresh();}catch(e){$('class-submission').textContent='This attempt has not submitted yet. Your local work is kept.';$('class-retry').hidden=false;fail(e.message);throw e;}finally{c.submitting=false;app.controls();}}
...
async function enter(room,teacher=false,initial=false){
 ...
 app.select(d.clipId);
 if(teacher){$('class-generated-code').value=room;$('class-student-link').value=base+'?room='+room;$('class-teacher').hidden=false;summary(d);if(d.runs.length){panel();app.compare(d.runs);}await historyData();message('Share the student link with your class.');}
 else{
  ...
  $('class-submission').textContent=c.participant?`Browser participant P${c.participant}. `+(c.closed?'Submissions are closed.':c.submitted?'Previous attempts are kept. You may record another attempt.':'Listen independently; completed attempts submit automatically.'):'Visitor view: the class comparison is available; no participant record was created.';
 $('class-receipts').textContent=(d.receipts||[]).sort((a,b)=>a.attempt-b.attempt).map(r=>`${room} · P${c.participant} · A${r.attempt} · ${new Date(r.submittedAt).toLocaleString()}`).join('\n');
...
$('class-copy-code').onclick=async()=>{try{await navigator.clipboard.writeText(c.room);message('Activity code copied. Give this code to your students.');}catch{$('class-generated-code').focus();$('class-generated-code').select();message('Code selected. Copy with your keyboard.');}};
