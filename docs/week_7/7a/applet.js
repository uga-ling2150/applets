...
const initialRoom=new URLSearchParams(location.hash.slice(1)).get('room')||new URLSearchParams(location.search).get('room');
const key=()=>`ling2150-7a:${window.BC_CLASSROOM?.room||initialRoom||"offline"}${window.BC_CLASSROOM?.mode==='teacher'||(!window.BC_CLASSROOM?.room&&new URLSearchParams(location.hash.slice(1)).has('key'))?':teacher':''}:${clip.id}`;
