...
async function start(practice){
 const cloud=window.BC_CLASSROOM||{};if(active||!ready||cloud.initializing||cloud.mode==='teacher'||cloud.mode==='student'&&(cloud.submitted||cloud.completed||cloud.closed)||(!practice&&runs.length>=core.MAX_RUNS))return;
 audio.pause();reviewUntil=null;active={id:uid(),clipId:clip.id,marks:[],exposed:revealed||$('transcript-details').open,practice,position:0};audio.currentTime=0;lastSave=-1;$('comparison').hidden=true;$('round-title').textContent=practice?'Practice — first 15 seconds, not saved':`Participant ${cloud.participant||runs.length+1}`;
...
window.BC_APP={snapshot:()=>({clip,runs:personalRuns||runs,active:!!active}),controls,select:(id)=>{const next=clips.find(c=>c.id===id);if(!next)throw Error('This recording is not available.');clip=next;$('clip').value=clips.indexOf(clip);load();},compare:(records)=>{if(window.BC_CLASSROOM?.mode==='student'&&personalRuns===null)personalRuns=runs;runs=records.map(r=>core.validateRun(r,clip));revealed=true;renderComparison();$('comparison-title').focus();},last:()=>(personalRuns||runs).at(-1)};
