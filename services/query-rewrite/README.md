...
8. Set `docs/week_5/5c/config.js` `apiBase` to the verified HTTPS Worker URL. The API uses public CORS (`Access-Control-Allow-Origin: *`) so the course website, local HTTP previews and local HTML files can call it without cookies or browser credentials. Keep the HTML's companion JavaScript/CSS assets when previewing locally. Repeat browser interaction and paid axe Pro scans after frontend changes. Add the verified applet link to the course index, then publish the frontend to the course's normal GitHub Pages branch.

## Operational limits

Students receive an automatic signed session without accounts or API keys. On September 18, the owner authorized removal of the origin allowlist to support local review. Any origin can now call this public API; link obscurity is not access control. Signed sessions are still required for rewrites, but can be renewed; the global Durable Object budget is the actual shared application ceiling. No per-IP blocking is used, to avoid penalizing a classroom sharing one Wi-Fi egress. Caps: 24 calls/session, 360/day UTC, 240/minute, 60 in flight. These are configurable operational defaults, not measured service limits. A single global coordinator stores counters and active leases, not conversation text.
