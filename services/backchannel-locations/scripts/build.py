...
s=page.read_text()
s=re.sub(r'<link rel="stylesheet" href="../style.css(?:\?[^"]*)?">',lambda m:'<style>'+ (docs/'style.css').read_text()+'</style>',s)
...
for name,content in [('data.js',data),('core.js',(assets/'core.js').read_text()),('applet.js',(assets/'applet.js').read_text()),('classroom.js',(assets/'classroom.js').read_text()),('live-clock.js',(assets/'live-clock.js').read_text()),('live.js',(assets/'live.js').read_text())]:
 s=re.sub(r'<script src="7a/'+re.escape(name)+r'(?:\?[^"]*)?"(?: defer)?></script>',lambda m:'<script>'+content.replace('</script','<\\/script')+'</script>',s)
s=s.replace('href="?mode=live"','href="https://uga-ling2150.github.io/applets/week_7/7A_backchannel_locations.html?mode=live"')
s=s.replace('href="7A_backchannel_locations.html"','href="#main"')
